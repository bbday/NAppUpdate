    ...
    public sealed class UpdateManager
    {
        ...
        private UpdateManager()
        {
            ...
            Config = new NauConfigurations
            {
                ...
                UpdateExecutableName = "NAppUpdateR.exe", // Naming it updater.exe seem to trigger the UAC, and we don't want that
            };
        ...
        {
            ApplyUpdates(relaunchApplication, false);
        }
        ...
        /// <returns>True if successful (unless a restart was required</returns>
        public void ApplyUpdates(bool relaunchApplication, bool updaterDoLogging)
        {
            if (IsWorking)
                throw new InvalidOperationException("Another update process is already in progress");
            ...
            lock (UpdatesToApply)
            {
                using (WorkScope.New(isWorking => IsWorking = isWorking))
                {
                    ...
                    if (hasColdUpdates)
                    {
                        ...
                        NauIpc.ExtractUpdaterFromResource(Config.TempFolder);
                        ...
                        var info = new ProcessStartInfo
                        {
                            ...
                            Arguments =
                                            string.Format(@"""{0}"" {1}", Config.UpdateProcessName,
