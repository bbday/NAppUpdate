    ...
    internal static class AppStart
    {
        ...
        private static void PerformUpdates()
        {
            ...
            foreach (var assemblyPath in availableAssemblies)
            {
                ...
                if (assemblyPath.Equals(Assembly.GetExecutingAssembly().Location, StringComparison.InvariantCultureIgnoreCase) || assemblyPath.EndsWith("NAppUpdateR.dll")
                    || assemblyPath.EndsWith("NAppUpdateR.exe"))
                {
